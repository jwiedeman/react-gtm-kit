...
function Products() {
  ...
  const [checkoutStep, setCheckoutStep] = createSignal<CheckoutStep>('browse');
  const [lastOrderId, setLastOrderId] = createSignal('');

  const cartTotal = createMemo(() => cart().reduce((sum, item) => sum + item.price * item.quantity, 0));
  ...
  const styles = {
    ...
    cardH3: { margin: '0 0 0.25rem', 'font-size': '1rem' },
    brand: { color: '#6c757d', 'font-size': '0.8rem', margin: '0 0 0.25rem' },
    price: { 'font-size': '1.125rem', 'font-weight': '600', color: '#27ae60', margin: '0 0 0.25rem' },
    category: { color: '#6c757d', 'font-size': '0.75rem', margin: '0 0 0.75rem' },
    cardButton: {
      width: '100%',
      background: '#3498db',
      color: 'white',
    ...
    checkoutPanel: { background: '#f8f9fa', padding: '2rem', 'border-radius': '8px', 'max-width': '500px' },
    cartItem: {
      display: 'flex',
    ...
    cartSummary: { 'margin-top': '1.5rem', 'padding-top': '1rem', 'border-top': '2px solid #e9ecef' },
    total: { 'font-size': '1.25rem', 'font-weight': '600', 'margin-bottom': '1rem' },
    ...
    formMock: { background: 'white', padding: '1rem', 'border-radius': '4px', 'margin-bottom': '1rem' },
    formMockP: { margin: '0.25rem 0', color: '#666' },
    grandTotal: { 'font-weight': '600', color: '#333', 'margin-top': '0.5rem' },
    completePanel: {
      background: '#f8f9fa',
      padding: '2rem',
      'border-radius': '8px',
    ...
    orderId: { color: '#666', 'font-family': 'monospace', 'margin-bottom': '1rem' }
  };
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        ...
        <Show when={cartCount() > 0 && checkoutStep() === 'browse'}>
      ...
      <Show when={checkoutStep() === 'browse'}>
        <div style={styles.grid}>
          <For each={products}>
            {(product, index) => (
              <div style={styles.card} onClick={() => viewProduct(product, index())}>
      ...
      <Show when={checkoutStep() === 'shipping'}>
        <div style={styles.checkoutPanel}>
      ...
      <Show when={checkoutStep() === 'payment'}>
        <div style={styles.checkoutPanel}>
          ...
          <div style={styles.formMock}>
            ...
            <p style={{ ...styles.formMockP, ...styles.grandTotal }}>
              Total: ${(cartTotal() * 1.08 + 5.99).toFixed(2)}
            </p>
          </div>
      ...
      <Show when={checkoutStep() === 'complete'}>
