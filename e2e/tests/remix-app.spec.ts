...
import { startRemixAppServer, type RemixAppServer } from '../apps/remix-app/server';

const getDataLayer = async <T>(page: Page): Promise<T[]> => {
  return page.evaluate(() => {
    const layer = (window as unknown as { remixDataLayer?: unknown[] }).remixDataLayer;
...
test.describe('Remix App example', () => {
  ...
  test.beforeAll(async () => {
    server = await startRemixAppServer();
  });
  ...
  test('renders GTM script and initializes data layer', async ({ page }) => {
    ...
    const scriptLocator = page.locator('head script[data-gtm-container-id="GTM-REMIXAPP"]');
    await expect(scriptLocator).toHaveCount(1);
    await expect(scriptLocator).toHaveAttribute('src', /l=remixDataLayer/);
  ...
  test('navigates between pages with Remix Router', async ({ page }) => {
    await page.goto(server.url, { waitUntil: 'networkidle' });
  ...
  test('tracks page_view events via useTrackPageViews', async ({ page }) => {
    await page.goto(server.url, { waitUntil: 'networkidle' });
